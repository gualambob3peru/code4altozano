...
<h3>Listado de Bancos de <?php echo $empresa["nombre"] ?></h3>

<p>
    <a href="admin/empresa/agregarBanco/<?php echo $empresa["id"] ?>" class="btn btn-warning"><i class="bi bi-plus-lg"></i> Agregar Nº de cuenta</a>
</p>
...
<table class="table table-bordered">
    ...
    <?php foreach ($bancos as $key => $value) : ?>
        <tr> 
            <td><?= $value->banco_descripcion; ?></td>
            <td><?= $value->nroCuenta; ?></td>
            <td><?= $value->descripcion_moneda." ".$value->simbolo_moneda; ?></td>
            <td>
                <a href="admin/<?= $table ?>/editarBanco/<?= $empresa["id"] ?>/<?= $value->id ?>" class="btn btn-info"><i class="bi bi-pencil"></i></a>
...
<script>
    ...
    for(let i=0; i<btnEliminarAll.length;i++){
        btnEliminarAll[i].onclick = function(){
            ...
            document.getElementById('btnOkEliminar').setAttribute('href','admin/<?= $table?>/eliminarBanco/'+id);
            modalEliminar.show();
