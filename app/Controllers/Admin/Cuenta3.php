...
class Cuenta3 extends BaseController
{
    private $db = "";
    private $table = "cuenta3";
    private $nombre = "cuenta3";
    private $lista = "cuenta3";
    private $dataView = [];
    private $model = "";
    ...
    public function agregar(){
        ...
        if ($this->request->getPost('submit')) {
            $rules = [
                'codigo' => 'required|validateCodigoCuenta3',
                'descripcion' => 'required',
            ];

            $errors = [
                'codigo' => [
                    'validateCodigoCuenta3' => 'Este código ya está registrado'
                ]
            ];
    ...
    public function ajaxEditar(){
        ...
        if ($_POST) {
            ...
            if($f_model["codigo"] == $this->request->getVar('codigo'))
                ...
                $rules["codigo"] = 'required|validateCodigoCuenta3';
    ...
    public function ajaxGetCuenta3(){
        $cuentas = (new Cuenta3Model())->where("idCuenta",$this->request->getVar('idCuenta'))->where("estado","1")->findAll();
        echo json_encode(array("response"=>$cuentas));
    }
    
    public function ajaxGetClase(){
        $cuentas = $this->db->table("cuenta3")
            ->select("cuenta3.id,cuenta3.descripcion,cuenta3.codigo,cuenta1.descripcion cuenta1_descripcion")
            ->join("cuenta2","cuenta2.id = cuenta3.idCuenta")
            ->join("cuenta1","cuenta1.id = cuenta2.idCuenta")
            ->join("clasecosto","clasecosto.id = cuenta1.idCuenta")
            ->where("cuenta3.estado","1")
            ->where("clasecosto.id",$_POST["idCuenta"])
            ->get()->getResult();
